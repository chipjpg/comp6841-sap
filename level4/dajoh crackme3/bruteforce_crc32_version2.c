...
int find_collision_from_list(char *str, unsigned char *resource, unsigned int lookuptable[256]) {
	unsigned int checksum = 0;
...
int main(int argc, char *argv[]) {
	...
	if (argc > 1) {
		if (!strcmp(argv[1], "--random")) {
			found = find_collision_from_random(str, resource, lookuptable);
		}
		else if (!strcmp(argv[1], "--list")) {
			found = find_collision_from_list(str, resource, lookuptable);
		}
